...
def test_retraction_deletes_close_match():
    # "forget pineapple" vs its target fact: live bge cosine 0.769 -> DELETE
    # (DELETE bar 0.72: above unrelated 0.38-0.52 and topical 0.65-0.70)
    op = D("Forget that I said I love pineapple on pizza", None,
           [_cand("User loves pineapple on pizza", 0.769)])
    assert op["event"] == "DELETE", op


def test_retraction_at_below_bar_does_not_delete():
    # retraction intent + no close target (0.68 < 0.72) -> must NOT delete;
    # a miss stores an (harmless) duplicate, never data loss.
    op = D("Forget that I said I love pineapple on pizza", None,
           [_cand("User loves spicy indian food", 0.68)])
    assert op["event"] == "ADD", op


def test_retraction_at_mild_overlap_does_not_delete():
    # retraction intent + mild topical overlap (0.55) -> must NOT delete
    op = D("Forget that I said I love pineapple on pizza", None,
           [_cand("User loves spicy indian food", 0.55)])
    assert op["event"] == "ADD", op

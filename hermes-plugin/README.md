...
> **Version:** this guide targets **memlite ≥ v1.3.0** (adds tiered L0/L1
...
uv pip install --python ~/.hermes/hermes-agent/venv/bin/python \
  "memlite @ git+https://github.com/AnkitArya/memlite.git@v1.3.0"
```
...
> Pin the version you want (`@v1.3.0`) or omit `@…` for latest `main`. The
...
Because memlite implements `post_setup(hermes_home, config)` (v1.3.0+), this
